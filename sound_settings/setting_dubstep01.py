...
def set_sound(note,sound_a,sampling,duration) :
    ...
    s0 = sin_out      (sound_a,duration,note - 36 ,1.0,sampling)

    s1 = sin_out      (sound_a,duration,note - 60 ,1.0,sampling)
    ...
    sb = fourier_trans_ratio(s3,Freq(sound_a,note),4,20,sampling)

    sc = Mix(sb,s2,0.5)

    sd = Modulate(sc,sa,0.01,0.0)
